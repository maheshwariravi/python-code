...
def GetMaxStockPrices(fileObject):
    ...
    companies = csv_reader.fieldnames[2:]   # First two columns are discard 
...
Year & Month
    ...
    for row in csv_reader:
        ...
        current = dict((comp, Peak(year, month, float(row[comp]))) for comp in 

companies)
        GetMAX = dict((comp, max(GetMAX [comp], current[comp])) for comp in 
...
def main():  # Can take a input_csv importing modules
